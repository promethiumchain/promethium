...
// promethium is the official command-line client for Promethium.
package main
...
const (
	clientIdentifier = "promethium" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, gitDate, "the promethium command line interface")
...
func init() {
	...
	app.Action = promethium
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2019 The promethium Authors"
...
// promethium is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func promethium(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local promethium node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Promethium node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
