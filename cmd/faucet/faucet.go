...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Promethium protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Promethium and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Promethium address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Promethium light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Promethium protocol stack
	client *ethclient.Client   // Client connection to the Promethium chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Promethium light client protocol
...
// close terminates the Promethium connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Promethium address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/promethiumchain/promethium/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Promethium address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Promethium address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Promethium address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Promethium address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Promethium address,
