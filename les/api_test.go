...
import (
	...
	"github.com/promethiumchain/promethium/p2p/simulations"
	"github.com/promethiumchain/promethium/p2p/simulations/adapters"
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	promethium, err := eth.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(promethium, &config)
	if err != nil {
		return nil, err
	}
	promethium.AddLesServer(server)
	return promethium, nil
}
