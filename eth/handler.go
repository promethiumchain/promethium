...
import (
	...
	"github.com/promethiumchain/promethium/eth/fetcher"
...
// NewProtocolManager returns a new Promethium sub protocol manager. The Promethium sub protocol manages peers capable
// with the Promethium network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Promethium peer", "peer", id)

	// Unregister the peer from the downloader and Promethium peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Promethium protocol")
	...
	log.Info("Promethium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Promethium peer connected", "name", p.Name())

	// Execute the Promethium handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Promethium handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Promethium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Promethium message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the Promethium sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Promethium network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
