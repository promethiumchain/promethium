...
// entire API provided by promethium to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the Promethium protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the Promethium protocol to
...
// Node represents a Geth Promethium node instance.
type Node struct {
	node *node.Node
}
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the Promethium protocol if requested
	if config.EthereumEnabled {
		...
		if err := rawStack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			return nil, fmt.Errorf("promethium init: %v", err)
		}
...
// GetEthereumClient retrieves a client to access the Promethium subsystem.
