...
// Package promethium contains the simplified mobile APIs to promethium.
//
// The scope of this package is *not* to allow writing a custom Promethium client
// with pieces plucked from promethium, rather to allow writing native dapps on
...
// from the promethium repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original promethium version.
